/*
  ...
  1. New Storage Buckets
    - `carousel`: For hero section carousel images
...
CREATE POLICY "Public Access" ON storage.objects FOR SELECT USING (bucket_id = 'carousel');
...
CREATE POLICY "Auth Upload Projects" ON storage.objects FOR INSERT 
WITH CHECK (
  bucket_id = 'projects' 
  AND auth.role() = 'authenticated'
);

CREATE POLICY "Auth Delete Projects" ON storage.objects FOR DELETE USING (
  bucket_id = 'projects'
  AND auth.role() = 'authenticated'
);
...
CREATE POLICY "Auth Delete Products" ON storage.objects FOR DELETE USING (
  bucket_id = 'products'
  AND auth.role() = 'authenticated'
);